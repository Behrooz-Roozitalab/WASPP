...
def operate_system(wayPoints):
  ...
  for wp in range(len(wayPoints)):
    ...
    messages = [str(gpsData_start[0]) + ',' + str(gpsData_end[0]) + ',' +
...
def main():
  ...
  parser.add_argument('-sf', '--simulationflag', help='simulation flag (0: operation, 1: simulation)', type=int)
  parser.add_argument('-w', '--waypoints', help='waypoints file location (csv)') 
  args = parser.parse_args()
  config = vars(args)

  # If simulation flag value is other than 0, use default value (1)
  if config['simulationflag']==0:
    simFlag = config['simulationflag']
  else:
    simFlag = 1
